...
def get_spatial_fragments_GMS(
    ...
    # video=video[:,int(video.shape[1]*0.1):int(video.shape[1]*0.9), int(video.shape[2]*0.1):int(video.shape[2]*0.9)]
    ...
    for i, hs in enumerate(hgrids):
        for j, ws in enumerate(wgrids):
            for t in range(dur_t // aligned):
                ...
                m = m+1
                # cv2.rectangle(nor_image, (int(h_so), int(w_so)), (int(h_eo), int(w_eo)), (255, 0, 0), 2)
...
def get_spatial_fragments_5_5(
    ...
    target_video = torch.zeros(video.shape[:-2] + size).to(video.device)
    m = 0
    for i, hs in enumerate(hgrids-1):
        for j, ws in enumerate(wgrids-1):
            for t in range(dur_t // aligned):
                t_s, t_e = t * aligned, (t + 1) * aligned
                h_s, h_e = i * fsize_h, (i + 1) * fsize_h
                w_s, w_e = j * fsize_w, (j + 1) * fsize_w
...
def get_spatial_fragments_2_2_64(
    video,
    fragments_h=2,
    fragments_w=2,
    fsize_h=64,
...
def get_spatial_fragments_2_2_32(
    video,
    fragments_h=6,
    fragments_w=6,
    fsize_h=32,
